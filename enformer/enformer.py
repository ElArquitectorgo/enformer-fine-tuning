...
class Enformer(snt.Module):
  ...
  def __init__(self,
    ...
    heads_channels = {'human': human_head, 'mouse': mouse_head}
    dropout_rate = 0.4
...
class Sequential(snt.Module):
  ...
  def __call__(self, inputs: tf.Tensor, is_training=True, **kwargs):
    outputs = inputs
...
class Residual(snt.Module):
  ...
  def __call__(self, inputs: tf.Tensor, is_training=True, *args,
               **kwargs) -> tf.Tensor:
